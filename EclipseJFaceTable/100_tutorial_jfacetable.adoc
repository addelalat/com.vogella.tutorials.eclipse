...
We will build an Eclipse RCP application which displays data of persons in a JFace table.
Each person is displayed in one individual row.
This tutorial the basic setup of a JFace Table.
...
Create a new RCP Project `de.vogella.jface.tableviewer` using the "RCP application with a view" as a template.
Create a package "de.vogella.jface.tableviewer.model" and the following class "Person".
...
The Person `class` represents the data model for this example.
...
Create the `ModelProvider` class which is a in-memory representation of your data.
